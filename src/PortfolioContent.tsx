...
export function Contact() {
  ...
  return <section id="contact-details" className="section contact-section"><SectionLabel number="06">What’s next?</SectionLabel><div className="contact-heading"><h2>Have something<br />in mind<span className="accent">?</span></h2><a className="contact-arrow" href={`mailto:${data.personal.email}`} aria-label="Email Samyak"><ArrowUpRight /></a></div><div className="contact-layout"><div><p className="large-copy">Let’s give it a first version.</p><p>I take on a small number of freelance projects alongside my work at UBS.</p><div className="email-line"><a href={`mailto:${data.personal.email}`}>{data.personal.email}</a><button className="icon-button" onClick={copy} aria-label="Copy email address">{copied === 'Email copied' ? <Check size={17} /> : <Copy size={17} />}</button></div><p className="form-note" role="status">{copied}</p><div className="social-links">{[['GitHub', data.personal.socials.github], ['LinkedIn', data.personal.socials.linkedin], ['X', data.personal.socials.twitter], ['Instagram', data.personal.socials.instagram], ['Sponsor', data.personal.socials.sponsor]].map(([label, href]) => <External key={label} href={href}>{label}</External>)}</div></div><form onSubmit={submit} className="contact-form"><div className="form-pair"><label htmlFor="contact-name">Your name<input id="contact-name" name="name" autoComplete="name" placeholder="Name" required /></label><label htmlFor="contact-email">Your email<input id="contact-email" name="email" type="email" autoComplete="email" placeholder="you@example.com" required /></label></div><label htmlFor="contact-message">What are you working on?<textarea id="contact-message" name="message" rows={3} placeholder="The idea, the challenge, or just a hello." required /></label><button className="primary-button" type="submit">Create email draft <ArrowUpRight size={18} /></button><p className="form-note" role="status">{drafted ? 'Your email app was requested. Review and send the draft there, or email me directly.' : 'Opens your email app with a draft ready to review.'}</p></form></div></section>;
}
